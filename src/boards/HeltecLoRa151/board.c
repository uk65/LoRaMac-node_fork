...
void BoardInitMcu (void)
{
    if (McuInitialized == false) {
        ...
        // NVM-Reset bei Programmstart
        GpioInit(&Nvm_Reset, NVM_RESET, PIN_INPUT, PIN_PUSH_PULL, PIN_PULL_UP, 1);
    ...
    //SpiInit(&NRF24L01.Spi, SPI_2, RF24_MOSI, RF24_MISO, RF24_SCLK, NC);
    //NRF24L01IoInit();
...
// Vdda value with which temperature sensor has been calibrated in production (+-10 mV).
...
#define COMPUTE_TEMPERATURE(TS_ADC_DATA, VDDA_APPLI)                                            \
    (((((((int32_t)((TS_ADC_DATA * VDDA_APPLI) / VDDA_TEMP_CAL) - (int32_t) TEMP30_CAL_ADDR)) * \
        (int32_t)(110 - 30)) << 8) / (int32_t)(TEMP110_CAL_ADDR - TEMP30_CAL_ADDR)) + (30 << 8))
...
// ARM executes the function when waking up
