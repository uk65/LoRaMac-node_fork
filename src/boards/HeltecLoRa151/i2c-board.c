...
LmnStatus_t I2cMcuWriteBuffer (I2c_t *obj, uint8_t deviceAddr, uint8_t *buffer, uint16_t size)
{
    ...
    status = (HAL_I2C_Master_Transmit(&I2cHandle, deviceAddr, buffer, size, 2000) == HAL_OK) 
        ? LMN_STATUS_OK
        : LMN_STATUS_ERROR;
...
LmnStatus_t I2cMcuReadBuffer (I2c_t *obj, uint8_t deviceAddr, uint8_t *buffer, uint16_t size)
{
    ...
    status = (HAL_I2C_Master_Receive(&I2cHandle, deviceAddr, buffer, size, 2000) == HAL_OK)
        ? LMN_STATUS_OK 
        : LMN_STATUS_ERROR;
...
LmnStatus_t I2cMcuWriteMemBuffer (I2c_t *obj, uint8_t deviceAddr, uint16_t addr, uint8_t *buffer, uint16_t size)
{
    ...
    status = (HAL_I2C_Mem_Write(&I2cHandle, deviceAddr, addr, memAddSize, buffer, size, 2000) == HAL_OK)
        ? LMN_STATUS_OK 
        : LMN_STATUS_ERROR;
...
LmnStatus_t I2cMcuReadMemBuffer (I2c_t *obj, uint8_t deviceAddr, uint16_t addr, uint8_t *buffer, uint16_t size)
{
    ...
    status = (HAL_I2C_Mem_Read(&I2cHandle, deviceAddr, addr, memAddSize, buffer, size, 2000) == HAL_OK)
        ? LMN_STATUS_OK
        : LMN_STATUS_ERROR;
...
LmnStatus_t I2cMcuWaitStandbyState (I2c_t *obj, uint8_t deviceAddr)
{
    ...
    status = (HAL_I2C_IsDeviceReady(&I2cHandle, deviceAddr, 300, 4096) == HAL_OK)
        ? LMN_STATUS_OK
        : LMN_STATUS_ERROR;
